...
#include<util/delay.h>          // 시간 지연함수를 선언
...
PORTD|=~0xFB;                	// 스위치를 땟을 때 다른 스위치를 누르기위해서 초기로 |마스킹해준다.
...
PORTD|=~0xF7;                  	// 스위치를 땟을 때 다른 스위치를 누르기위해서 초기로 |마스킹해준다.
...
PORTD|=~0xEF;               	// 스위치를 땟을 때 다른 스위치를 누르기위해서 초기로 |마스킹해준다.
...
void main()                     //메인함수의 선언
{
   ...
   while(1)                  	//무한루프문 진입
   {
      ...
      DDRA=0xFF;            	// PORTA는 모든비트에서 출력을 설정
      data=keypad();          	// data는 keypad함수의 return값을 받음
      PORTA=array[data];     	// PORTA는 배열의 data값으로 출력
   }
} 
